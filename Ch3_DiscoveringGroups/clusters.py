...
def getheight(clust):
    # Is this an enpoint? Then the height is just 1
    if clust.left == None and clust.right == None: return 1

    # Otherwise the height is the same of the height of
    # each branch
    return getheight(clust.left) + geteheight(clust.right)
    ...
    for t in range(100):
        ...
        for j in range(len(rows)):
            ...
            for i in range(k):
                ...
                if d < distancecters[bestmatch], row): bestmatch = i

            bestmatches[bestmatch].append(j)

        # if the results are the same as last time, this is complete
        if bestmatches == lastmatches: break
        latstmatches = bestmatches
...
def scaledown(data, distance = pearson, rate = 0.01):
    ...
    for m in range(0, 1000):
        ...
        for k in range(n):
            for j in range(n):
                ...
                # Each point needs to be moved away from or towards the outer
        ...
        if lasterror and lasterror < toptalerror: break
    ...
    def draw2d(data, labels, jpeg = 'md2d.jpg'):
